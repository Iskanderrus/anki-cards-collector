...
export function collectorIdentityQuery(lexicalUnitId: string): string {
  // The encoded identity is hex-only, and anchored tag regex prevents Anki's
  // normal tag hierarchy/wildcards from matching child or lookalike tags.
  return `tag:re:^${collectorIdentityTag(lexicalUnitId)}import type {
  CollectedItem,
  CollectorSemanticField,
  ExportFieldMapping,
  ExportProfile,
