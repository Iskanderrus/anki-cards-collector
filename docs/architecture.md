...
Because the Manifest V3 background worker may be suspended at any time, the active staged batch is reconstructed from a versioned `chrome.storage.session` payload containing only the batch identity and staged evidence. This storage is transient to the browser session and remains outside the corpus/backup/export model. On worker revival, the background reruns the batch classification against the current repository rather than restoring stale dispositions from storage.
